...
// ---- BDI Report Card Component ----
export default function BDIReportCard({ loading, error, report, onRetry }) {
  const printRef = useRef(null);
  const [downloading, setDownloading] = useState(false);
  const [downloadError, setDownloadError] = useState(null);

  if (loading) return <LoadingState label="Loading BDI report..." />;
  if (error) return <ErrorState message={error} onRetry={onRetry} />;
  if (!report) return null;

  const { user, bdi_report, saved_report } = report;
  if (!bdi_report) return null;
  const { total_score, max_score, level, question_details, summary } = bdi_report;
  const severityColor = getBDISeverityColor(level?.label);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.setFontSize(20); pdf.setTextColor(47, 68, 121); pdf.text("BDI Depression Assessment Report", margin, yPos); yPos += 12;
      pdf.setFontSize(12); pdf.setTextColor(100, 100, 100); pdf.text("Beck Depression Inventory Results", margin, yPos); yPos += 8;
      addLine();
      ...
      if (question_details) {
        Object.entries(question_details).forEach(([qId, detail]) => {
          ...
          const qColor = getBDIScoreColor(qScore).replace("#", "");
      ...
      pdf.save(`BDI_Report_${safeName}_${date}.pdf`);
    } catch (err) {
  ...
  return (
    <ReportCardWrapper
      icon={<AlertTriangle size={14} className="sm:w-[16px] sm:h-[16px] md:w-[17px] md:h-[17px] text-blue-700" />}
      iconBg="bg-blue-100"
      title="BDI Depression Assessment"
      subtitle="Beck Depression Inventory result"
      badge={level ? { 
        text: level.label, 
    ...
    >
      <div ref={printRef} className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-5 md:space-y-6 bg-white">
        <ReportHeader title="BDI Depression Assessment Report" user={user} savedReport={saved_report} />
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
            {Object.entries(BDI_SEVERITY_CONFIG).map(([label, config]) => (
        ...
        <div>
          ...
          <div className="space-y-1.5 sm:space-y-2">
            {question_details && Object.entries(question_details).map(([qId, detail]) => {
              ...
              const scoreColor = getBDIScoreColor(score);
              return (
